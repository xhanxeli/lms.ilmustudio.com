...
class AccountingController extends Controller
{
    public function index()
    {
        ...
        // Load orders for charge account transactions
        foreach ($accountings as $accounting) {
            if ($accounting->type == Accounting::$addiction && 
                $accounting->type_account == Accounting::$asset &&
                $accounting->description == trans('public.charge_account')) {
                // Find the order associated with this charge account transaction
                $order = Order::where('user_id', $accounting->user_id)
                    ->where('is_charge_account', true)
                    ->where('total_amount', $accounting->amount)
                    ->whereBetween('created_at', [$accounting->created_at - 60, $accounting->created_at + 60])
                    ->where('status', '!=', Order::$pending)
                    ->first();
                
                $accounting->charge_order = $order;
            }
        }
    ...
    public function charge(Request $request)
    {
        ...
        if (!empty($request->file('attachment'))) {
            $rules['attachment'] = 'mimes:jpeg,png,jpg,pdf|max:5120';
        }
    ...
    private function handleUploadAttachment($user, $file)
    {
        ...
        if (strtolower($extension) === 'pdf') {
            ...
            // For image files, use Image::make to process
            $img = Image::make($file);
    ...
    public function chargeAccountInvoice($id)
    {
        $this->authorize("panel_financial_summary");

        $userAuth = auth()->user();

        // Get the accounting record (document) - must belong to the user
        $document = Accounting::where('id', $id)
            ->where('user_id', $userAuth->id)
            ->with(['user', 'creator'])
            ->first();
        ...
        // Verify it's a charge account transaction using the same logic as the view
        $isChargeAccount = ($document->type == Accounting::$addiction && 
                           $document->type_account == Accounting::$asset &&
                           empty($document->webinar_id) && 
                           empty($document->bundle_id) && 
                           empty($document->product_id) && 
                           empty($document->meeting_time_id) && 
                           empty($document->subscribe_id) && 
                           empty($document->promotion_id) && 
                           empty($document->registration_package_id) && 
                           empty($document->installment_payment_id) &&
                           $document->store_type != Accounting::$storeManual &&
                           !$document->is_registration_bonus &&
                           !$document->is_cashback);

        if (!$isChargeAccount) {
            abort(404);
        }

        // Use the same view as admin
        $data = [
            'document' => $document,
            'currency' => currency()
        ];
