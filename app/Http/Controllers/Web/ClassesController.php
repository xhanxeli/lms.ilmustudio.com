...
class ClassesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Filter by subscription categories if user has active subscriptions
        // Only filter if ALL subscriptions have specific categories assigned
        // If any subscription has no categories, user can see all categories
        if (auth()->check()) {
            $activeSubscribes = Subscribe::getActiveSubscribes(auth()->id());
            if ($activeSubscribes->isNotEmpty()) {
                // Collect all allowed category IDs from all active subscriptions
                $allCategoryIds = [];
                $hasUnrestrictedSubscription = false;
                
                foreach ($activeSubscribes as $subscribe) {
                    if ($subscribe->categories && $subscribe->categories->count() > 0) {
                        $allCategoryIds = array_merge($allCategoryIds, $subscribe->categories->pluck('id')->toArray());
                    } else {
                        // If any subscription has no category restrictions, show all categories
                        $hasUnrestrictedSubscription = true;
                        break;
                    }
                }
                
                // Only filter if all subscriptions have category restrictions
                if (!$hasUnrestrictedSubscription && !empty($allCategoryIds)) {
                    $categoryIds = array_unique($allCategoryIds);
                    $webinarsQuery = $webinarsQuery->whereIn("{$this->tableName}.category_id", $categoryIds);
                }
            }
        }
        ...
        if (!empty($search)) {
            $webinarsQuery->whereTranslationLike('title', "%{$search}%");
        }
