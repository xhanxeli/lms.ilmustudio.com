...
class WebinarsController extends Controller
{
    ...
    public function purchases(Request $request)
    {
        ...
        // Optimize: Load all sales at once instead of N+1 queries
        $webinarIds = $webinars->pluck('id')->toArray();
        $sales = Sale::where('buyer_id', $user->id)
            ->whereIn('webinar_id', $webinarIds)
            ->where('type', 'webinar')
            ->whereNull('refund_at')
            ->select('webinar_id', 'created_at') // Only select needed columns
            ->get()
            ->keyBy('webinar_id'); // Key by webinar_id for easy lookup
        
        foreach ($webinars as $webinar) {
            if (isset($sales[$webinar->id])) {
                $webinar->purchast_date = $sales[$webinar->id]->created_at;
            }
        }
