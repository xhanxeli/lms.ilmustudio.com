...
{
    private function inferFileTypeFromPath(?string $filePath): ?string
    {
        ...
        // Map common extensions to File::$fileTypes
        if ($extension === 'pdf') {
            return 'pdf';
        }

        if (in_array($extension, ['ppt', 'pptx', 'pps', 'ppsx'])) {
            return 'powerpoint';
        }

        if (in_array($extension, ['mp3', 'wav', 'm4a', 'aac', 'ogg', 'flac'])) {
            return 'sound';
        }

        if (in_array($extension, File::$videoTypes)) {
            return 'video';
        }
        ...
        // Default for uploads when unknown
        return 'document';
    }
    ...
    public function store(Request $request)
    {
        ...
        // If uploading a local file and file_type is omitted, infer it from the file_path extension.
        // This avoids 422 validation errors when the UI doesn't prompt for file type.
        if (($data['storage'] ?? null) === 'upload' && empty($data['file_type']) && !empty($data['file_path'])) {
            $data['file_type'] = $this->inferFileTypeFromPath($data['file_path']);
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Same inference for update requests.
        if (($data['storage'] ?? null) === 'upload' && empty($data['file_type']) && !empty($data['file_path'])) {
            $data['file_type'] = $this->inferFileTypeFromPath($data['file_path']);
        }
