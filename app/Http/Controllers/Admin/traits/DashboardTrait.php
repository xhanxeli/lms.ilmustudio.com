...
trait DashboardTrait
{
    public function dailySalesTypeStatistics()
    {
        ...
        // Calculate total amount of subscription sales for today (incl. renewal-success rows)
        $todaySalesAmount = Sale::whereAdminDashboardSuccessfulSubscribePlan()
    ...
    public function getIncomeStatistics()
    {
        ...
        // Calculate total amount of subscription sales for the current month (incl. renewal-success rows)
        $monthlySalesAmount = Sale::whereAdminDashboardSuccessfulSubscribePlan()
            ->whereBetween('created_at', [$beginOfMonth, $endOfMonth])
            ->sum('total_amount');

        return [
            'monthlySalesAmount' => $monthlySalesAmount ?? 0,
        ];
    }
    ...
    {
        // Calculate total amount of all subscription plan sales (all time, incl. renewal-success rows)
        $totalSalesAmount = Sale::whereAdminDashboardSuccessfulSubscribePlan()
            ->sum('total_amount');
    ...
    public function getActiveSubscriptionPlanStatistics()
    {
        $metrics = Subscribe::getAdminDashboardActiveSubscriptionMetrics();
        $dates = $this->getAllDateStartAndEnd();
        $beginOfDay = $dates['today']['start'];
        $endOfDay = $dates['today']['end'];
        $metrics['todayNewSubscribePlansCount'] = Sale::whereAdminDashboardSuccessfulSubscribePlan()
            ->whereBetween('created_at', [$beginOfDay, $endOfDay])
            ->count();
    ...
    /**
     * Offline bank approvals + paid wallet top-ups via payment gateways (orders do not store which gateway;
     * this uses all successful payment_channel charge orders — typically Billplz when that is the enabled gateway).
     */
    public function getTotalWalletRechargeStatistics()
    {
        $manual = OfflinePayment::where('status', OfflinePayment::$approved)->sum('amount');
        $gateway = Order::query()
            ->where('is_charge_account', true)
            ->where('status', Order::$paid)
            ->where('payment_method', Order::$paymentChannel)
            ->sum('total_amount');

        return [
            'offlineBankTransferTotal' => (float) ($manual ?? 0),
            'billplzAndGatewayTotal' => (float) ($gateway ?? 0),
            'totalRecharged' => (float) (($manual ?? 0) + ($gateway ?? 0)),
        ];
    }
