...
class WebinarController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Process start_date for webinars (required) and courses/text_lessons (optional)
        ...
        } elseif ($data['type'] != Webinar::$webinar) {
            // For courses and text_lessons, set to null only if not provided (webinars require it)
        ...
        $webinar = Webinar::create([
            ...
            'live_now' => false, // Live now is managed via toggle in the list view
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!empty($data['start_date']) and $webinar->type == 'webinar') {
            ...
            // For courses and text_lessons, preserve existing start_date instead of setting to null
    ...
    public function toggleLiveNow(Request $request, $id)
    {
        $this->authorize('admin_webinars_edit');

        $webinar = Webinar::query()->findOrFail($id);

        // Only allow toggling for webinar type
        if ($webinar->type != Webinar::$webinar) {
            return response()->json([
                'status' => 'error',
                'message' => trans('admin/main.invalid_request')
            ], 400);
        }

        // Check if live_now has been on for more than 12 hours and auto-turn it off
        $currentTime = time();
        if ($webinar->live_now && !empty($webinar->live_now_at)) {
            ...
            if ($timeElapsed >= $twelveHoursInSeconds) {
                ...
                return response()->json([
                    'status' => 'success',
                    'message' => trans('public.request_success'),
                    'live_now' => false,
                    'auto_disabled' => true
                ]);
            }
        ...
        // Get the live_now value - handle boolean, string "true"/"false", or "1"/"0"
        $liveNowValue = $request->get('live_now', false);
        
        // Convert to boolean - handle various input formats
        $liveNow = false;
        if ($liveNowValue === true || $liveNowValue === 'true' || $liveNowValue === '1' || $liveNowValue === 1 || $liveNowValue === 'on') {
            $liveNow = true;
        }
        ...
        // Set timestamp when turning on, clear when turning off
        if ($liveNow) {
            $updateData['live_now_at'] = $currentTime;
        } else {
            $updateData['live_now_at'] = null;
        }
        ...
        return response()->json([
            'status' => 'success',
            'message' => trans('public.request_success'),
            'live_now' => (bool)$webinar->live_now
        ]);
    }
