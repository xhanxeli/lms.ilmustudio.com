   ...
   public function verify(Request $request, $gateway)
{
    try {
        ...
        if (!empty($order)) {
            $status = $request->get('status') === 'success' ? Order::$paying : Order::$fail;
            $order->update(['status' => $status]);

            if ($status === Order::$paying) {
                return redirect()->route('payment_success')->with('message', 'Payment successful!');
            } else {
                return redirect()->route('payment_fail')->with('message', 'Payment failed.');
            }
        }
    } catch (\Exception $e) {
    ...
    return redirect('/cart')->with('message', 'Unable to process the payment.');
}
