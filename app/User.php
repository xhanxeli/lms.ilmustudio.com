...
class User extends Authenticatable
{
    ...
    private function getEffectiveSubscriptionExpirationTimestamp(Sale $subscribeSale, \App\Models\Subscribe $subscribe): ?int
    {
    ...
    {
        // Cache this expensive operation for 2 minutes
        $cacheKey = "all_purchased_webinars_ids_{$this->id}";
        return \Cache::remember($cacheKey, 120, function () {
            $userBoughtWebinarsIds = [];
            // Optimize: Only select needed columns and use pluck instead of get
            $userBoughtWebinars = Sale::query()
                ->where('buyer_id', $this->id)
                ->whereNotNull('webinar_id')
                ->whereNull('refund_at')
                ->where('type', 'webinar') // Add type filter to use index
                ->select('id', 'webinar_id', 'payment_method') // Only select needed columns
                ->get();

            foreach ($userBoughtWebinars as $sale) {
                // For subscription purchases, check if SubscribeUse is active
                if ($sale->payment_method == Sale::$subscribe) {
                    $hasActiveUse = \App\Models\SubscribeUse::where('user_id', $this->id)
                        ...
                        ->whereHas('sale', function ($q) {
                            $q->where('type', Sale::$subscribe)->whereNull('refund_at');
                        })
                        ->exists();
                    
                    if ($hasActiveUse) {
                        $userBoughtWebinarsIds[] = $sale->webinar_id;
                    }
                } else {
                    // For non-subscription purchases, add directly
                    $userBoughtWebinarsIds[] = $sale->webinar_id;
                }
            }

            return array_unique($userBoughtWebinarsIds);
        });
    }
    ...
    {
        // Cache this expensive operation for 2 minutes
        $cacheKey = "purchased_courses_ids_{$this->id}";
        return \Cache::remember($cacheKey, 120, function () {
            $webinarIds = [];
            $bundleIds = [];

            // Optimize: Only select needed columns and use indexes
            $sales = Sale::where('buyer_id', $this->id)
                ...
                ->whereNull('refund_at')
                // Only include sales that still grant access
                ->where('access_to_purchased_item', true)
                ->select('id', 'webinar_id', 'bundle_id', 'payment_method') // Only select needed columns
                ->get();
        ...
        foreach ($sales as $sale) {
            if ($sale->payment_method == Sale::$subscribe) {
                ...
                if (!empty($sale->webinar_id)) {
                    // Check if SubscribeUse is active
                    $activeUse = \App\Models\SubscribeUse::where('user_id', $this->id)
                        ->where('webinar_id', $sale->webinar_id)
                        ->where('active', true)
                        ->where(function ($q) {
                    ...
                    if ($activeUse) {
                        // Find the subscription purchase sale (not the subject purchase sale)
                        $subscribeSale = Sale::where('buyer_id', $this->id)
                            ->where('type', Sale::$subscribe)
                            ->where('subscribe_id', $activeUse->subscribe_id)
                            ->whereNull('refund_at')
                            ->latest('created_at')
                            ->first();
                        ...
                        if ($subscribeSale && $subscribeSale->subscribe) {
                            ...
                            $effectiveExpiration = $this->getEffectiveSubscriptionExpirationTimestamp($subscribeSale, $subscribe);
    ...
    /**
     * Get purchased courses IDs including those with active SubscribeUse records
     * Only includes webinars where SubscribeUse is active and not expired
     */
    public function getPurchasedCoursesIdsWithActiveSubscriptions()
    {
        $webinarIds = $this->getPurchasedCoursesIds();

        // Include courses with active SubscribeUse (subscription purchases) that are not expired
        $activeSubscribeUses = \App\Models\SubscribeUse::where('user_id', $this->id)
            ->where('active', true)
            ->where(function ($q) {
                $q->whereNull('expired_at')
                    ->orWhere('expired_at', '>', time());
            ...
            ->whereNotNull('webinar_id')
            ->whereHas('sale', function ($q) {
                $q->where('type', \App\Models\Sale::$subscribe)
                    ->whereNull('refund_at');
            })
            ->with(['sale.subscribe'])
            ->get();
        ...
        foreach ($activeSubscribeUses as $use) {
            ...
            if ($subscribeSale && $subscribeSale->subscribe) {
                ...
                if (!$isExpired) {
                    $activeWebinarIds[] = $use->webinar_id;
                }
            }
        }
        
        if (!empty($activeWebinarIds)) {
            $webinarIds = array_unique(array_merge($webinarIds, $activeWebinarIds));
        }
