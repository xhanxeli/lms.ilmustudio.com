...
{
    // Cache the admin panel URL prefix to avoid database queries on every route load
    // This is critical because routes/admin.php calls this function at the top level
    $cacheKey = 'admin_panel_url_prefix';
    
    try {
        $prefix = Cache::remember($cacheKey, 3600, function () {
            ...
            try {
                ...
                \Log::warning('Failed to get admin panel URL from database, using default', [
                    'error' => $e->getMessage()
                ]);
                return 'admin';
            }
        ...
        \Log::warning('Failed to get admin panel URL from cache, using default', [
            'error' => $e->getMessage()
        ]);
        return 'admin';
    }
    ...
    if (!empty($notificationTemplate)) {
        ...
        if (empty($check) or !in_array($template, $ignoreDuplicateTemplates)) {
            ...
            if (env('APP_ENV') == 'production') {
                // Disable email notifications for groups - only send emails when user_id is set and type is not 'group'
                if ($type !== 'group' && !empty($user_id)) {
...
function fromAndToDateFilter($from, $to, $query, $column = 'created_at', $strToTime = true)
{
    if (!empty($from) and !empty($to)) {
        ...
        // Check if $to is at the start of a day (00:00:00)
        // If so, set it to end of day (23:59:59) to include all records from that day
        $toDate = date('Y-m-d', $to);
        $toStartOfDay = strtotime($toDate . ' 00:00:00');
        if ($to == $toStartOfDay) {
        ...
        if (!empty($to)) {
            ...
            // If to is at the start of a day, set it to end of day
            $toDate = date('Y-m-d', $to);
            $toStartOfDay = strtotime($toDate . ' 00:00:00');
            if ($to == $toStartOfDay) {
                $to = strtotime($toDate . ' 23:59:59');
