...
class InstallmentAccounting
{
    public function refundOrder($order)
    {
        ...
        // If this installment order is for a subscription, expire all SubscribeUse records
        if (!empty($order->subscribe_id)) {
            $subscribeUses = \App\Models\SubscribeUse::where('user_id', $order->user_id)
                ->where('subscribe_id', $order->subscribe_id)
                ->where('installment_order_id', $order->id)
                ->where('active', true)
                ->get();
            ...
            \Log::info('Expired SubscribeUse records after installment order refund', [
                'order_id' => $order->id,
                'user_id' => $order->user_id,
