            ...
            try {
                $static = cache()->remember('settings.' . $name, 24 * 60 * 60, function () use ($name) {
                    try {
                        ...
                        // If database query fails (timeout, connection error, etc.), log and return null
                        \Log::error('Failed to load setting from database', [
                            'setting_name' => $name,
                            'error' => $e->getMessage()
                        ]);
                        return null;
                ...
                // If cache fails, log and set to null
                \Log::error('Failed to load setting from cache', [
                    'setting_name' => $name,
                    'error' => $e->getMessage()
                ]);
                $static = null;
