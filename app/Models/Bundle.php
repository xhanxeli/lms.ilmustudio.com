...
class Bundle extends Model implements TranslatableContract
{
    ...
    public function checkUserHasBought($user = null, $checkExpired = true): bool
    {
        ...
        if (!empty($user)) {
            ...
            // IMPORTANT: Also verify there's a corresponding Sale record for this bundle
            if (!$hasBought) {
                // Get ALL active SubscribeUse records for this bundle
                ...
                foreach ($activeUses as $activeUse) {
                    // CRITICAL: Verify there's a corresponding Sale record for this bundle
                    // SubscribeUse alone is not enough - there must be a Sale record
                    $bundleSale = \App\Models\Sale::where('buyer_id', $user->id)
                        ->where('bundle_id', $this->id)
                        ->where('payment_method', \App\Models\Sale::$subscribe)
                        ->whereNull('refund_at')
                        ->first();
                    
                    // If no Sale record exists for this bundle, deny access
                    // SubscribeUse records without corresponding Sales are invalid
                    if (empty($bundleSale)) {
                        continue; // Skip this use - no valid sale
                    }
                    
                    // Verify the subscription is still valid
                    $subscribe = \App\Models\Subscribe::find($activeUse->subscribe_id);
                    if ($subscribe) {
                        // Find the subscription sale to check expiration
                        ...
                        if ($subscribeSale) {
                            ...
                            $usedDays = $createdAt->diffInDays($now);
                            // If subscription is not expired (usedDays < subscribe->days), grant access
