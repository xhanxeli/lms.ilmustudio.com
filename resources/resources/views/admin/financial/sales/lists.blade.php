                                ...
                                <table class="table table-striped font-14">
                                    ...
                                    @foreach($sales as $sale)
                                        <tr>
                                            ...
                                            @php
                                                // Check if this refunded sale was used for renewal extension
                                                $isRenewalExtension = false;
                                                $extendedSale = null;
                                                if (!empty($sale->refund_at) && $sale->type == \App\Models\Sale::$subscribe) {
                                                    $extendedSale = \App\Models\Sale::where('buyer_id', $sale->buyer_id)
                                                        ...
                                                        ->where('custom_expiration_date', '>', $sale->created_at)
                                                        ->orderBy('created_at', 'desc')
                                                        ->first();
                                                    $isRenewalExtension = !empty($extendedSale);
                                                }
                                            @endphp
                                            <td>
                                                @if($isRenewalExtension)
                                                    {{-- Show as successful for renewal sales --}}
                                                    <span class="text-success">{{ trans('admin/main.success') }}</span>
                                                @elseif(!empty($sale->refund_at))
                                            ...
                                            <td>
                                                ...
                                                @can('admin_sales_refund')
                                                    @if($isRenewalExtension and $extendedSale)
                                                        {{-- Show refund button for renewal sales - refund the extended sale --}}
                                                        @include('admin.includes.delete_button',[
                                                                'url' => getAdminPanelUrl().'/financial/sales/'. $extendedSale->id .'/refund',
