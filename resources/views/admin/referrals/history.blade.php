        ...
        <div class="section-body">
            ...
            <div class="row">
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <form method="get" action="{{ getAdminPanelUrl() }}/referrals/history" class="d-flex align-items-end justify-content-between w-100">
                                <div class="d-flex align-items-end">
                                    <div class="form-group mb-0 mr-2">
                                        <label class="input-label">{{ trans('admin/main.search') }} ({{ trans('admin/main.affiliate_user') }} / {{ trans('admin/main.referred_user') }})</label>
                                        <input type="text" class="form-control" name="search" value="{{ $search ?? '' }}" placeholder="{{ trans('admin/main.search_by_name_or_email') }}" style="width: 300px;">
                                    </div>
                                    <div class="form-group mb-0 mr-2">
                                        <label class="input-label">{{ trans('admin/main.start_date') }}</label>
                                        <input type="date" class="form-control" name="from" value="{{ $from ?? '' }}" placeholder="{{ trans('admin/main.start_date') }}">
                                    </div>
                                    <div class="form-group mb-0 mr-2">
                                        <label class="input-label">{{ trans('admin/main.end_date') }}</label>
                                        <input type="date" class="form-control" name="to" value="{{ $to ?? '' }}" placeholder="{{ trans('admin/main.end_date') }}">
                                    </div>
                                    <div class="form-group mb-0 mr-2">
                                        <label class="input-label">&nbsp;</label>
                                        <button type="submit" class="btn btn-primary d-block">{{ trans('admin/main.show_results') }}</button>
                                    </div>
                                    @if(!empty($from) || !empty($to) || !empty($search))
                                        <div class="form-group mb-0">
                                            <label class="input-label">&nbsp;</label>
                                            <a href="{{ getAdminPanelUrl() }}/referrals/history" class="btn btn-secondary d-block">{{ trans('admin/main.clear') }}</a>
                                        </div>
                                    @endif
                                </div>
                                @can('admin_referrals_export')
                                    <div class="ml-auto">
                                        <label class="input-label">&nbsp;</label>
                                        <a href="{{ getAdminPanelUrl() }}/referrals/excel?type=history{{ !empty($search) ? '&search=' . urlencode($search) : '' }}{{ !empty($from) ? '&from=' . urlencode($from) : '' }}{{ !empty($to) ? '&to=' . urlencode($to) : '' }}" class="btn btn-primary d-block">{{ trans('admin/main.export_xls') }}</a>
                                    </div>
                                @endcan
                            </form>
                        </div>
