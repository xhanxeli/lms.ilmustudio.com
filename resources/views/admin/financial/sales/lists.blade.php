                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label class="input-label">{{ trans('admin/main.search') }} ({{ trans('admin/main.id') }} / {{ trans('admin/main.email') }} / {{ trans('admin/main.affiliate') }} / {{ trans('admin/main.referral_code') }})</label>
                ...
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            @endcan
                            <a href="{{ getAdminPanelUrl() }}/financial/sales/api-access" class="btn btn-outline-primary ml-2">{{ trans('admin/main.sales_api_keys') }}</a>
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped font-14">
                                    <tr>
                                        ...
                                        <th>{{ trans('admin/main.affiliate') }}</th>
                                    ...
                                    @foreach($sales as $sale)
                                        <tr>
                                            ...
                                            <td>
                                                <span class="{{ $sale->getAdminFinancialSalesListStatusCssClass() }}">{{ $sale->getAdminFinancialSalesListStatusText() }}</span>
                                            </td>

                                            <td>
                                                @if($sale->type == \App\Models\Sale::$subscribe)
                                                    @if(!empty($sale->affiliate_user))
                                                        {{ $sale->affiliate_user->full_name ?? '' }}
                                                        @if(!empty($sale->affiliate_user->id))
                                                            <div class="text-primary text-small font-600-bold">ID : {{ $sale->affiliate_user->id }}</div>
                                                        @endif
                                                    @else
                                            ...
                                            <td>
                                                @if($sale->type == \App\Models\Sale::$subscribe)
                                                    @if(!empty($sale->affiliate_code))
                                                        <span class="badge badge-info">{{ $sale->affiliate_code }}</span>
                                                    @else
                                                        <span class="text-muted">-</span>
                                                    @endif
                                                @else
                                            ...
                                            <td>
                                                @can('admin_sales_invoice')
                                                    @if((!empty($sale->webinar_id) or !empty($sale->bundle_id)) and $sale->type != \App\Models\Sale::$webinar)
                                                ...
                                                @can('admin_sales_refund')
                                                    @if(!empty($sale->webinar_id) and empty($sale->refund_at) and $sale->access_to_purchased_item)
                                                        @include('admin.includes.delete_button',[
                                                                'url' => getAdminPanelUrl().'/financial/sales/'. $sale->id .'/cancel',
                                                                'tooltip' => trans('admin/main.cancel_course'),
                                                                'btnIcon' => 'fa-ban'
                                                            ])
                                                    @endif
                                                    
                                                    @if($sale->getAdminRenewalExtensionPartnerSale())
                                                        {{-- Show refund button for renewal sales - refund the partner (extended) sale --}}
                                                        @include('admin.includes.delete_button',[
                                                                'url' => getAdminPanelUrl().'/financial/sales/'. $sale->getAdminRenewalExtensionPartnerSale()->id .'/refund',
                                                                'tooltip' => trans('admin/main.refund'),
                                                                'btnIcon' => 'fa-times-circle'
                                                            ])
                                                    @elseif(empty($sale->refund_at) and $sale->type == \App\Models\Sale::$subscribe)
                                                        {{-- Show refund button for regular subscription sales --}}
                                                        @include('admin.includes.delete_button',[
                                                                'url' => getAdminPanelUrl().'/financial/sales/'. $sale->id .'/refund',
                                                                'tooltip' => trans('admin/main.refund'),
                                                                'btnIcon' => 'fa-times-circle'
                                                            ])
                                                    @elseif(empty($sale->refund_at) and $sale->payment_method != \App\Models\Sale::$subscribe)
                                                        @include('admin.includes.delete_button',[
                                                                'url' => getAdminPanelUrl().'/financial/sales/'. $sale->id .'/refund',
                                                                'tooltip' => trans('admin/main.refund'),
                                                                'btnIcon' => 'fa-times-circle'
