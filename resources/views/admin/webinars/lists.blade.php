                                            ...
                                            @if($classesType == 'webinar')
                                                <td class="text-center">
                                                    <div class="custom-control custom-switch">
                                                        <input type="checkbox" 
                                                               class="custom-control-input live-now-toggle" 
                                                               id="liveNowSwitch{{ $webinar->id }}" 
                                                               data-webinar-id="{{ $webinar->id }}"
                                                               {{ $webinar->live_now ? 'checked' : '' }}>
                                                        <label class="custom-control-label" for="liveNowSwitch{{ $webinar->id }}"></label>
                                                    </div>
                                                </td>
                                            @endif
                                                ...
                                                <div class="btn-group dropdown table-actions">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-left text-left webinars-lists-dropdown">
                                                        ...
                                                        @can('admin_webinars_delete')
                                                            @php
                                                                $isLmsManager = !empty(auth()->user()->role_name) && 
                                                                    (stripos(auth()->user()->role_name, 'manager') !== false || 
                                                                     auth()->user()->role_name === 'LMS_Manager' ||
                                                                     auth()->user()->role_name === 'lms_manager' ||
                                                                     auth()->user()->role_name === 'LMS Manager');
                                                            @endphp
                                                            @if(!$isLmsManager)
                                                                @include('admin.includes.delete_button',[
    ...
    <script>
        (function ($) {
            ...
            $(document).ready(function() {
                $('.live-now-toggle').on('change', function() {
                    var $toggle = $(this);
                    var webinarId = $toggle.data('webinar-id');
                    ...
                    $.ajax({
                        url: '{{ getAdminPanelUrl() }}/webinars/' + webinarId + '/toggle-live-now',
                        method: 'POST',
                        data: {
                            live_now: isChecked,
                            _token: '{{ csrf_token() }}'
                        ...
                        success: function(response) {
                            if (response.status === 'success') {
                                ...
                                $originalToggle.prop('checked', !isChecked);
                                $.toast({
                                    heading: '{{ trans("public.request_failed") }}',
                                    text: response.message || '{{ trans("public.request_failed") }}',
                                    icon: 'error',
                                    position: 'top-right',
                                    stack: false
                                });
