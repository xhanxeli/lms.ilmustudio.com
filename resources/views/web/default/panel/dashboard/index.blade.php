...
@push('styles_top')
    ...
    <style>
        .live-now-icon-blink {
            animation: live-now-blink 1.5s ease-in-out infinite;
        }
        
        @keyframes live-now-blink {
            0%, 100% {
                opacity: 1;
            }
            50% {
...
@section('content')
    ...
    <section class="dashboard">
        <div class="row">
            ...
            <div class="col-12 col-lg-3 mt-35">
                ...
                <a href="@if($authUser->isUser()) /panel/webinars/purchases @else /panel/financial/sales @endif" class="dashboard-stats rounded-sm panel-shadow p-10 p-md-20 d-flex align-items-center mt-15 mt-md-30">
                    <div class="stat-icon monthly-sales">
                        @if($authUser->isUser())
                            <svg class="live-now-icon-blink" width="40" height="40" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="d-flex flex-column ml-15">
                        <span class="font-30 text-secondary">{{ !empty($monthlySalesCount) ? handlePrice($monthlySalesCount) : (!empty($liveNowCount) ? $liveNowCount : 0) }}</span>
                        <span class="font-16 text-gray font-weight-500">{{ $authUser->isUser() ? trans('webinars.live_now') : trans('panel.monthly_sales') }}</span>
                    </div>
                </a>
            </div>

            <div class="col-12 col-lg-3 mt-35">
                <a href="/panel/webinars/purchases/expired-subjects" class="dashboard-stats rounded-sm panel-shadow p-10 p-md-20 d-flex align-items-center">
                    <div class="stat-icon support-messages">
                        <img src="/assets/default/img/icons/clock.svg" alt="">
                    </div>
                    <div class="d-flex flex-column ml-15">
                        <span class="font-30 text-secondary">{{ !empty($expiredSubjectsCount) ? $expiredSubjectsCount : 0 }}</span>
                        <span class="font-16 text-gray font-weight-500">Expired subject</span>
                    </div>
                </a>

                @if($authUser->isUser())
                    <a href="/panel/financial/subscribes" class="dashboard-stats rounded-sm panel-shadow p-10 p-md-20 d-flex align-items-center mt-15 mt-md-30">
                        <div class="stat-icon comments">
                            <img src="/assets/default/img/icons/comment.svg" alt="">
                        </div>
                        <div class="d-flex flex-column ml-15">
                            <span class="font-30 text-secondary">{{ !empty($subscriptionPlansCount) ? $subscriptionPlansCount : 0 }}</span>
                            <span class="font-16 text-gray font-weight-500">Subscription Plans</span>
                        </div>
                    </a>
                @else
                    <a href="/panel/webinars/comments" class="dashboard-stats rounded-sm panel-shadow p-10 p-md-20 d-flex align-items-center mt-15 mt-md-30">
                        <div class="stat-icon comments">
                            <img src="/assets/default/img/icons/comment.svg" alt="">
                        </div>
                        <div class="d-flex flex-column ml-15">
    ...
    <section class="">
        <div class="d-flex align-items-start align-items-md-center justify-content-between flex-column flex-md-row">
            <h1 class="section-title" style="margin-top: 30px; margin-bottom: 20px;">Subjects</h1>
        </div>
    </section>
<iframe id="myIframe" border-radius="5px" frameBorder="0" src="https://lms.ilmustudio.com/classes?sort=newest" width="100%" scrolling="no"></iframe>
<script>
