                            ...
                            <table class="table text-center custom-table">
                                ...
                                @foreach($accountings as $accounting)
                                    <tr>
                                        ...
                                        <td class="text-center align-middle">
                                            @php
                                                // Check if this is a charge account transaction
                                                $isChargeAccount = ($accounting->type == \App\Models\Accounting::$addiction && 
                                                                   $accounting->type_account == \App\Models\Accounting::$asset &&
                                                                   empty($accounting->webinar_id) && 
                                                                   empty($accounting->bundle_id) && 
                                                                   empty($accounting->product_id) && 
                                                                   empty($accounting->meeting_time_id) && 
                                                                   empty($accounting->subscribe_id) && 
                                                                   empty($accounting->promotion_id) && 
                                                                   empty($accounting->registration_package_id) && 
                                                                   empty($accounting->installment_payment_id) &&
                                                                   $accounting->store_type != \App\Models\Accounting::$storeManual &&
                                                                   !$accounting->is_registration_bonus &&
                                                                   !$accounting->is_cashback);
                                            @endphp
                                            @if($isChargeAccount)
                                                <a href="/panel/financial/summary/charge-account/{{ $accounting->id }}/invoice" target="_blank" class="btn btn-sm btn-primary">
                                                    <i class="fa fa-download"></i> {{ trans('public.invoice') }}
                                                </a>
                                            @endif
                                        </td>
                                    </tr>
