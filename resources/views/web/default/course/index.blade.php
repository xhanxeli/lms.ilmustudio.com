...
@section('content')
        ...
        <div class="row">
            <div class="col-12 col-lg-8">
                <div class="course-content-body user-select-none">
                    <div class="course-body-on-cover text-white"><button class="btn btn-primary" onclick="window.history.back()">Back</button><br>
                      <br>
                        <h1 class="font-30 course-title">
                            {{ $course->title }}
                        </h1>
                        @if($course->type == 'webinar' && !empty($course->live_now))
                            <div class="live-now-badge mt-15 d-inline-flex align-items-center">
                        ...
                        {{-- Ratings and profile name hidden on frontend --}}
                    ...
                    <div class="mt-35">
                        <ul class="nav nav-tabs bg-secondary rounded-sm p-15 d-flex align-items-center justify-content-between" id="tabs-tab" role="tablist">
                            ...
                            {{-- Reviews tab hidden on frontend --}}
                            {{-- <li class="nav-item">
                        ...
                        <div class="tab-content" id="nav-tabContent">
                            ...
                            {{-- Reviews tab content hidden on frontend --}}
    ...
    @if($hasBought or !empty($course->getInstallmentOrder()))
        <a target="_blank" href="{{ $course->getLearningPageUrl() }}" class="btn btn-primary">{{ trans('update.go_to_learning_page') }}</a>
    @elseif(!empty($course->price) and $course->price > 0)
        <!-- Add to Cart Button -->
        @if($course->subscribe)
            @if($hasBought)
                <a target="_blank" href="{{ $course->getLearningPageUrl() }}" class="btn btn-primary mt-20">{{ trans('update.go_to_learning_page') }}</a>
            @else
    ...
    @else
        <!-- Enroll for Free -->
        <a href="/course/{{ $course->slug }}/free" class="btn btn-primary">
    ...
    @endif
                ...
                {{-- Profile cards hidden on frontend --}}
                {{-- organization --}}
